...
by delegating to tooling/rulebook_check.py.
...
# Map system_id → ruleset name in tooling/rulebook_check.py.
...
# checks (palette, type-scale, etc.) live in tooling/rulebook_check.py
...
def check_compliance(
    ...
    if rb_module is None:
        return Result.failure(Error(
            "INTERNAL",
            "Could not import tooling/rulebook_check.py. Ensure the project root layout is intact.",
...
# ─── Internal: import tooling/rulebook_check.py ─────────────────────

_RB_MODULE_CACHE: Any = None


def _import_rulebook_check_module():
    """Import tooling/rulebook_check.py as a module.
    ...
    rb_path = project_root() / "tooling" / "rulebook_check.py"
