...
The registry is a YAML file at the project root (registry.yaml) with
this shape:
    ...
    systems:
      - id: swiss
        ...
        library_root: swiss-library # optional; defaults to <id>-library
      - id: wireframe
        spec: wireframe-spec.yaml
        status: available
        library_root: wireframe-library

`spec` and `library_root` are paths relative to the project root.
...
class RegistryEntry:
    ...
    def from_dict(cls, d: dict, default_library_root: str | None = None) -> "RegistryEntry":
        ...
        return cls(
            ...
            library_root=d.get("library_root", default_library_root or f"{d['id']}-library"),
        )
...
def register_system(spec_dir: str | Path, replace: bool = False) -> Result:
    ...
    defaults to <id>-library (if it exists at project root) or the spec
    ...
    library_dir = root / f"{system_id}-library"
    library_root = f"{system_id}-library" if library_dir.exists() else str(spec_path.parent.relative_to(root) if spec_path.parent.is_relative_to(root) else spec_path.parent)
